...
/**
 * Filters for sass/scss-files.
 *
 * @param {string[]} files
 * @returns {string[]}
 */
ParallelSass.prototype.filterFiles = function (files) {
...
ParallelSass.prototype.run = function(files) {
    ...
    if (files.length < this.options.threads) {
        this.options.threads = files.length;
    }
...
ParallelSass.prototype.onWorkerMessage = function(payload) {
    ...
    switch (payload.action) {
        ...
        case 'finished':
            ...
            logger.log(Verbosity.debug, 'childrenPending ' + this.childrenPending);
